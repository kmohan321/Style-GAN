...
class DISCRIMINATOR(nn.Module):
  def __init__(self,in_channels):
      ...
      for i in range(len(channels_factors)-1,0,-1):
        ...
        self.rgb_layers.append(WSConv2d(3,in_conv,kernel_size=1,stride=1,padding=0))
  ...
  def forward(self,x,alpha,step):
      ...
      if step == 0 :
        x = self.minibatch_std(out)
        return self.final_block(x).reshape(x.shape[0],-1)

      
      downscaled = self.act(self.rgb_layers[cur_step + 1](self.down(x)))
      out = self.down(self.blocks[cur_step](out))
      
      x = self.transition(downscaled,out,alpha)
